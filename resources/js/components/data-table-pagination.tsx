...
export function DataTablePagination<TData>({
    ...
    return (
      <div className="flex flex-col gap-2 items-center md:flex-row md:items-center md:justify-between px-2">
...
interface ShowingCurrentProps {
    rowCount: number,
    pageIndex: number,
    pageSize: number,
}

function ShowingCurrent({rowCount, pageIndex: n, pageSize: b} : ShowingCurrentProps){
    const upperCurrentPage = 1 + (n)*b
    const tempLower = b * (n+1)
    const lowerCurrentPge = rowCount > tempLower ? tempLower : rowCount
    const showingRowCountString = `Showing ${upperCurrentPage} - ${lowerCurrentPge} of ${rowCount}`
    return(
