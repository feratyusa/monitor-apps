...
export default function DashboardInvoiceContent({counts} : DashboardInvoiceCountsProps) {
    ...
    return(
        <div className="flex flex-col gap-5">
            ...
            <IncreaseStatus label="Unpaid" current={counts.unpaid} last={counts.last_unpaid} reverse={true}/>
            <IncreaseStatus label="Overdue" current={counts.overdue} last={counts.last_overdue} reverse={true}/>
        </div>
    )
}

function IncreaseStatus({label, current, last, reverse = false} : {label: string, current: number, last: number, reverse?: boolean})  {
